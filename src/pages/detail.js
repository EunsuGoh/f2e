...
function Detail() {
    ...
    const nftContractAddress = "0xd499fBE9B701db12801371d8CFE6eE342eB0bA32";
    ...
    const initContract = async (web3instance,contractABI, contractAddress)=>{
     ...
     const buyNFT = async () =>{
        await payNFT();
        const nftBuyResult = await axios.post(`http://127.0.0.1:3000/flower/market/buy?tokenid=${tokenid}`,{"account":account});
    ...
    const getNFTdetail = async () =>{
        const result = await axios.get(`http://127.0.0.1:3000/flower/market/detail?tokenid=${tokenid}`);
        return result.data
    }
    const handleExit = () =>{
      navigate(-1);
